...
void  serializeSHT(ivy_ser &res, int msgType, int reqType, int key, int data){
  res.open_struct();
...
int send_recv(ivy_binary_ser &res, int &fd, sockaddr_in &remaddr,int client, int req){
  stTimeOut.tv_sec = 2;
  stTimeOut.tv_usec = 5;
  
  // cout << client  << " SENDING "<< req <<"\n";
   ...
   if (sendto(fd, &res.res[0],res.res.size(), 0, (struct sockaddr *)&remaddr, slen)==-1) {
                 ...
                 FD_SET(fd, &rset);
                        ...
                        if ((bytes = recvfrom(fd,&buf[0],len,0,0,0)) < 0)
                            { std::cerr << client << " retry " << req << "\n";  retries = retries +1; return 0;}

                              // std :: cout << clientReqCnt <<" :: REPLY RECIEVED : Client " <<client << " , " << req << "\n";
                              clientReqCnt = clientReqCnt +1;
                              ...
                              totalLatency += elapsed.count();
															return 1;
                              // std::cout << "Msg(" << clientReqCnt <<") Elapsed time: " << elapsed.count() << " total =" << msgTime <<"\n";
}



void  get_server_address(int my_id, sockaddr_in &remaddr) {
...
void sendrecvloop(int tid, int numOfNodes,int fd){
  ...
  while(totalReq < num_reqs){
    int dst = rand() % numOfNodes;
    get_server_address(dst, remaddr);
    ivy_binary_ser res;
    ...
    string op;
		totalReq = totalReq + 1;
    if(opChance > 90){
      int randomValue = rand() % 1000; //out if 100
      op = "set("+ std::to_string(randomKey) + "," + std::to_string(randomValue)+ ")";
    
      serializeSHT(res,SERVER_MSG,SET,randomKey,randomValue);
      
    }else{
      op = "get(" + std::to_string(randomKey) + ")";
      serializeSHT(res,SERVER_MSG,GET,randomKey,0);
    }
		
		if(totalReq%1000 == 0){cout << "Client " << tid << "Req # " << totalReq << " :: " << num_reqs <<" \n";}
		
    int recieved = 0;
		
		while(recieved < 1){
			recieved = send_recv(res,fd,remaddr,tid,totalReq);
		}
...
FD_ZERO(&rset); 
  ...
  int numOfNodes = stoi(argv[1]); // how many server nodes are in the system
 ...
 auto finish = std::chrono::high_resolution_clock::now();
  ...
  cout << "total time : " << msgTime << " - clientReqCnt : " << clientReqCnt << "average = " << msgTime/clientReqCnt << "\n";
  cout << "total latency : " << totalLatency << " :: avg = " << totalLatency/clientReqCnt << "\n"  ;
